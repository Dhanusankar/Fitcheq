...
// Initialize PostgreSQL connection pool - same as in lib/db.ts
const pool = new Pool({
...
/**
 * Add exercises to the database directly
 */
async function addExercisesDirectly() {
  try {
    console.log('Starting to add exercises directly to the database...');
    ...
    // Still try to close the pool on error
    try {
      await pool.end();
    } catch (e) {
